...
use std::{borrow::Cow, collections::HashMap, convert::identity, fmt, iter::repeat, sync::Arc, time::Duration};
...
fn get_tile_keys_in_range(
    ...
    TileKey::range(from, to, zoom, min_level, max_level, health).iter().map(TileKey::to_smolstr).collect()
}
...
impl<'a> Intel<'a> {
    ...
    pub async fn get_entities_in_range(
        ...
        let params = get_entities_in_range::Params {
            inner: self,
            tiles: Mutex::new(
                tile_keys.into_iter().map(|id| (id, get_entities_in_range::TileState::Free)).collect::<HashMap<_, _>>(),
            ),
            api_version,
            csrftoken,
        };
...
mod tests {
    ...
    async fn login() -> super::Intel<'static> {
        ...
        if let Some(cookies) = env::var("COOKIES").ok() {
            intel
